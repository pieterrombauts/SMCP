    ...
    <div className={props.className}>
      { currEFN !== null && 
        <div key={props.efnID}>
          ...
          <hr />
          {currEFN._comments.map((comment, index) => {
            return(<React.Fragment key={props.efnID + "-" + index}><p className="efn-comment">{comment}</p><hr /></React.Fragment>)
          })}
        </div>
      }
      <Form.Control className="efn-comment-box" as="textarea" name="content" rows="5" ref={commentRef} />
      <Button className="efn-add-comment-btn" variant="primary" onClick={addEFNComment}>Add Comment</Button>
    </div>
  );
}
